...
**🔴 CRÍTICO de la sesión anterior — RESUELTO, no es un bug:** le pregunté a Matias directamente si él promovió la cuenta de prueba (`sld.claude.qa.20260826@mailinator.com`) a `role='admin'`. Confirmó que sí, y que también la marcó "destacada" a propósito. **No revertir el rol.** Autorizó explícitamente usar esa cuenta para QA de todo el negocio en adelante.

**Hecho esta sesión:**
- `4d...` `fix(perfil)`: `app/perfil/page.tsx` no traía `display_name` en el `select` y mostraba el email crudo en el `<h1>` y el avatar — mismo bug que ya se había arreglado en `header.tsx`. Aplicado el mismo fix. Verificado con `npm run build` real (antes esta sesión no tenía shell en el dispositivo; ahora sí).
- `sweep-ui.mjs` y `qa-walk-tmp.mjs` (sueltos sin trackear): `sweep-ui.mjs` se rescató a `scripts/sweep-ui.mjs` (ya commiteado) como herramienta de QA reusable. `qa-walk-tmp.mjs` era de un solo uso (debug de este mismo bug de `/perfil`, ya resuelto) → movido a `~/basura-sld/`, no versionado.
- **QA real con la cuenta de prueba** (Playwright contra el dev server, reusando el `storageState` de la sesión anterior en `/tmp/qa-storage.json` — seguía autenticado): flujos de comprador (favoritos, mi-barata, pedidos, mapa, feed, vecinos, ranking, radar, pulso, invitar) todos OK; menú de header segmentado por rol confirmado (compra + comerciante + sección "Administración" con Moderación/Verificación, visible solo por ser admin); los 3 tabs de `/admin?tab=overview|moderacion|verificacion` cargan bien; resto del nav de comerciante (Reels, Muro, Analytics, Mensajes, Reseñas, Seguidores, Sellitos, Soporte, Planes) con contenido real, no solo status 200.
- **Bug real encontrado y arreglado — hidratación rota en `/negocios`:** la tarjeta de negocio (`components/negocios-client.tsx`) tenía un `<a>` de WhatsApp anidado dentro del `<a>` de la tarjeta (HTML inválido) y un avatar (`<div>`) metido dentro de un `<p>`. El browser corregía el DOM y React tiraba errores de hidratación en cada carga de `/negocios`. Fix: la tarjeta pasó a ser un `<div>` con un Link "estirado" (`position:absolute` vía `style` inline — ver punto de abajo) hacia `/negocio/[slug]`, y WhatsApp + Favorito quedan como hermanos clickeables por encima (z-10), no anidados. Verificado con diagnóstico real en el browser (rect, `elementFromPoint`, click real): 0 errores de hidratación, la tarjeta entera sigue navegando, favorito y WhatsApp siguen funcionando independientes.
- **Bug de arquitectura CSS encontrado (no resuelto de raíz, documentado):** `app/globals.css` línea ~1133 tiene `a, [role="link"] { @apply relative; }` **fuera de cualquier `@layer`** — en Tailwind v4 (que usa `@layer theme,base,components,utilities`), una regla sin layer le gana a CUALQUIER utilidad de Tailwind (`.absolute`, `.fixed`, `.sticky`, etc.) aplicada a un `<a>`/`<Link>` en TODO el sitio, sin importar especificidad. Lo confirmé con `getComputedStyle` en el browser: `.absolute` en un `<Link>` se computaba como `position: relative`. Para el fix de arriba usé `style` inline (máxima prioridad, no depende de layers, cero riesgo de romper otras páginas). **Pendiente para una sesión dedicada:** envolver esa sección de `globals.css` ("ELEMENTO: Links y navegación") en `@layer components` o similar — pero antes hay que auditar si algún otro lugar del sitio depende (a sabiendas o no) de que esa regla gane siempre, porque cambiar el layering es un cambio de alcance global.
- **Bug real encontrado y arreglado — loop de fetch en `/dashboard`:** cada carga de `/dashboard` disparaba ~29 POST a `/api/tracked-links` en 4 segundos hasta pegarle al rate limit (429), aunque el usuario nunca abre el panel "QR de vidriera" (`components/dashboard/qr-vidriera.tsx`, colapsado en un `<details>` pero monta igual). Causa: `components/ui/toast.tsx` (`ToastProvider`) creaba la función `show` (y el objeto de contexto) nuevos en cada render, sin `useCallback`/`useMemo`; `QrVidriera` tiene un `useEffect` que depende de un `useCallback` que a su vez depende de `show`, así que cada re-render del provider (que envuelve toda la app) volvía a disparar el fetch. Fix en el origen: `show` con `useCallback([])`, el `value` del contexto con `useMemo`, y de paso el id de cada toast pasó de `Date.now()` (colisionaba si dos toasts salían en el mismo milisegundo → warning de key duplicada, la pista que llevó al bug) a un contador propio. Verificado real contando responses de red antes/después: 29 llamadas (mayoría 429) → 4 llamadas estables (400 legítimo, el negocio de prueba no está verificado, así que `create_tracked_link` rechaza correctamente — no es un bug, es la regla de negocio funcionando bien).
- Build + lint corridos de verdad esta sesión (antes no había shell en el dispositivo, ahora sí vía `mcp__remote-devices__desktop-control__run_shell`): **0 errores** en ambos, 516 warnings preexistentes (ninguno nuevo introducido).

**Pendiente para la próxima sesión:**
- `/dashboard/editar/[slug]` (botón "✏️ Editar" en `app/dashboard/page.tsx:173`) no se probó a fondo con contenido real — sí se confirmó que la ruta existe en el código, pero no llegué a cargarla con la cuenta de prueba dentro del tiempo de esta sesión.
- UX menor: el toast de error de `QrVidriera` dice "Probá de nuevo" aunque la causa (negocio no verificado) no se arregla reintentando — mensaje debería distinguir ese caso.
- `app/globals.css`: aparte de la limpieza de CSS muerto ya pendiente (ver FASE 3 más abajo), ahora también queda pendiente resolver de raíz el problema de layers sin `@layer` en la sección de links (ver arriba) — auditar primero quién depende de ese comportamiento antes de tocarlo.
- Sigue pendiente: "Negocios en Tendencia" (`trending_businesses`) sin componente que la muestre; FASE 2 SEO; limpieza pre-lanzamiento (ocultar/borrar cuenta+negocio de prueba cuando Matias decida que ya terminó de usarla para QA — por ahora sigue activa y autorizada).

---

## 0-BIS. HANDOFF URGENTE — sesión 2026-08-26 tarde/noche (contexto de la sesión anterior, ya resuelto salvo lo marcado)
