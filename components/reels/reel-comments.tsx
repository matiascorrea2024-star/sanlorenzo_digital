...
export default function ReelComments({ reelId, onClose, onCommentAdded }: {
  ...
  return (
    <div className="absolute inset-0 z-20 flex flex-col justify-end bg-[var(--scrim)]" onClick={onClose}>
      <div
        className="max-h-[70vh] rounded-t-[1.75rem] border-t border-[var(--ov-08)] bg-[var(--surface2)] p-1.5"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex max-h-[70vh] flex-col rounded-t-[1.375rem] border border-[var(--ov-05)] bg-[var(--card-inner)] p-4">
          <div className="mb-3 flex items-center justify-between">
            <p className="font-black text-[var(--text)]">Comentarios ({comments.length})</p>
            <button onClick={onClose} aria-label="Cerrar" className="rounded-full bg-[var(--ov-10)] p-1.5 hover:bg-[var(--ov-20)]">
              ...
              comments.map((c) => (
                <div key={c.id} className="flex items-start gap-2.5">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-sm"><span className="font-bold text-[var(--text)]">{c.sender_name}</span> <span className="text-[var(--text)]/80">{c.body}</span></p>
                  </div>
                </div>
              ))
            )}
