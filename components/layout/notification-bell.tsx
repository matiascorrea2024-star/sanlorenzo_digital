          ...
          <div className="fixed left-4 right-4 top-20 z-[60] sm:absolute sm:left-auto sm:right-0 sm:top-full sm:mt-2 sm:w-96 w-80 rounded-[1.5rem] border border-[var(--ov-06)] bg-[var(--ov-03)] p-1.5 shadow-2xl backdrop-blur-xl">
            <div className="rounded-[1.1rem] border border-[var(--ov-05)] bg-[var(--surface2)] p-4">
              <div className="mb-3 flex items-center justify-between">
                ...
                {count > 0 && <button onClick={marcarLeidas} className="text-xs font-bold text-[var(--accent-ink)] hover:text-[var(--accent-ink)]">Marcar leídas</button>}
              </div>
              {items.length === 0 ? (
                ...
                <div className="max-h-80 space-y-2 overflow-y-auto">
                  {items.map(n => (
                    <button key={n.id} onClick={() => irA(n)}
                      ...
                      <p className="mt-1 text-[10px] font-bold text-[var(--accent-ink)]">Tocar para ir →</p>
                    </button>
                  ))}
                </div>
              )}
      ...
      {toast && (
        <button onClick={() => { irA(toast); setToast(null); }}
          ...
          <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-[var(--accent)]/25 to-[var(--accent2)]/15">
            <MessageCircle className="h-4 w-4 text-[var(--accent-ink)]" />
          </span>
          <div className="text-left">
            ...
            <p className="text-[10px] font-bold text-[var(--accent-ink)]">Tocar para abrir →</p>
          </div>
        </button>
      )}
    </>
