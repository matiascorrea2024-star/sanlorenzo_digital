...
export default function BottomNav() {
  ...
  return (
    <nav aria-label="Navegación principal"
      className="fixed bottom-0 left-0 right-0 z-50 border-t border-[var(--line)] bg-[var(--bg)]/95 pb-[env(safe-area-inset-bottom)] backdrop-blur-xl md:hidden">
      <div className="pointer-events-none h-px bg-gradient-to-r from-transparent via-[var(--accent)]/40 to-transparent" />
        ...
        {ITEMS.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href}
              ...
              <span className={`relative flex h-7 w-9 items-center justify-center rounded-full transition-all duration-300 ${active ? "bg-[var(--accent)]/15" : ""}`}>
                ...
                {item.href === "/promociones" && nuevasHoy > 0 && (
                  <span
                    ...
                    className="absolute -right-1.5 -top-1 flex h-3.5 min-w-[14px] items-center justify-center rounded-full bg-[var(--accent)] px-0.5 text-[8px] font-black leading-none text-white"
                  >
