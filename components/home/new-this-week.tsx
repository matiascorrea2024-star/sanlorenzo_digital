  ...
  return (
    <section className="mx-auto max-w-4xl px-4 py-8">
      <Link href={`/negocio/${n.slug}`} key={n.id}
        ...
        <div className="flex items-center gap-4 rounded-[1.375rem] border border-white/[.06] bg-black/20 p-5 shadow-[inset_0_1px_1px_rgba(255,255,255,.06)] sm:p-6">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="truncate text-lg font-black text-white">{n.name}</p>
            <p className="truncate text-sm capitalize text-white/50">{n.category}{n.description ? ` · ${n.description}` : ""}</p>
          </div>
      ...
      {negocios.length > 1 && (
        <div className="mt-3 flex justify-center gap-1.5">
          {negocios.map((_, i) => (
            <span key={i} className={`h-1.5 rounded-full transition-all duration-300 ${i === idx ? "w-5 bg-emerald-400" : "w-1.5 bg-white/15"}`} />
          ))}
        </div>
      )}
    </section>
