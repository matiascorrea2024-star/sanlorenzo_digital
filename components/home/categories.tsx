...
export default function Categories({ active, onSelect }: { active: string | null; onSelect: (id: string | null) => void }) {
  return (
    <section id="categorias" className="mx-auto max-w-7xl px-4 py-12 sm:px-6">
      ...
      <div className="sld-no-scrollbar flex snap-x snap-mandatory gap-2 md:gap-3 overflow-x-auto pb-2 md:grid md:grid-cols-4 lg:grid-cols-8">
        {CATEGORIES.map((c, i) => (
          <button
            key={c.id}
            onClick={() => onSelect(active === c.id ? null : c.id)}
            data-spot className={`group relative min-w-[100px] md:min-w-[112px] snap-start overflow-hidden rounded-2xl border p-4 text-left transition duration-300 md:min-w-0 ${
              active === c.id
