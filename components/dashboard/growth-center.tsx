...
export default function GrowthCenter({ business, ofertasActivas, respuestaMedia }: Props) {
  ...
  return (
    <section className="rounded-[2rem] border border-white/5 bg-[#161314] p-6">
      ...
      <p className="mt-1 text-xs text-[#a99b86]">Checklist real de {business.name}: cada punto que falta es plata que no entra.</p>
      ...
      <ul className="mt-4 grid gap-1.5 sm:grid-cols-2">
        {items.map((i) => (
          <li key={i.key} className={`flex items-center gap-2 rounded-xl px-3 py-2 text-xs font-bold ${i.ok ? "bg-white/[.03] text-[#f7f3ec]" : "bg-white/[.03] text-[#a99b86]"}`}>
            {i.ok ? <CheckCircle2 className="h-4 w-4 shrink-0 text-[var(--ok)]" /> : <Circle className="h-4 w-4 shrink-0 text-[#7d6f5c]" />}
      ...
      {pendientes.length > 0 ? (
        <div className="mt-4 space-y-2 border-t border-white/5 pt-4">
          <p className="text-[10px] font-black uppercase tracking-[0.35em] text-[#7d6f5c]" style={{ fontFamily: "var(--font-display)" }}>Lo que falta, sin vueltas</p>
          {pendientes.map((i) => (
            <div key={i.key} className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-white/5 bg-white/[.02] px-3 py-2.5">
              <p className="min-w-0 flex-1 text-xs text-[#a99b86]">{i.tip}</p>
