...
export default function OnlineBadge({ lastSeen, size = "sm" }: { lastSeen: string | null | undefined; size?: "sm" | "md" }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 ${txt} font-bold ${online ? "text-emerald-300" : "text-white/40"}`}>
      <span className={`relative flex ${dot} shrink-0 rounded-full ${online ? "bg-emerald-400" : "bg-white/30"}`}>
