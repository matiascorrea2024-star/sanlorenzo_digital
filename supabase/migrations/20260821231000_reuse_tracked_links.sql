-- Evita crear una fila nueva cada vez que el comerciante abre Marketing.
...
begin
  ...
  if p_offer_id is not null then
    select business_id into v_offer_business_id from public.offers where id = p_offer_id and active = true;
  ...
  if not exists (
    ...
    where id = v_business_id and activo is distinct from false and status in ('verificado', 'reclamado')
