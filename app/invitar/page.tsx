...
export default function InvitarPage() {
  ...
  if (loading) return <main className="flex min-h-screen items-center justify-center bg-[var(--bg)] text-sm text-[var(--muted)]">Cargando…</main>;
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[var(--bg)] pb-24 text-[var(--text)]">
      ...
      <div className="relative z-10 mx-auto max-w-lg px-4 py-8 text-center sm:px-6">
        ...
        <>
        <div className="rounded-[2.5rem] border border-[var(--line-strong)] bg-[var(--surface)] p-8 shadow-2xl">
          <img src={qrUrl} alt="QR de invitación" className="mx-auto h-52 w-52 rounded-2xl bg-white p-3" />
          <p className="mt-5 break-all rounded-2xl border border-dashed border-[var(--accent)]/40 bg-[var(--accent)]/5 px-4 py-3 font-mono text-[11px] tracking-wide text-[var(--muted)]">{link}</p>
