  ...
  if (loading) {
    return (
      <main className="min-h-screen bg-[#0c0a0b] text-[#f7f3ec] pb-24">
        ...
        <div className="mx-auto max-w-5xl px-4 py-16 text-center text-[#a99b86]">Cargando...</div>
      </main>
    );
  }
    ...
    return (
      <main className="min-h-screen bg-[#0c0a0b] text-[#f7f3ec] pb-24">
        ...
        <div className="mx-auto max-w-5xl px-4 py-12 text-center">
          <div className="mx-auto max-w-md rounded-3xl border border-dashed border-white/10 bg-[#161314] p-10">
            <Package className="mx-auto h-16 w-16 text-[#7d6f5c] mb-4" />
            <p className="text-[#a99b86]">No hay productos seleccionados para comparar.</p>
  ...
  return (
    <main className="min-h-screen bg-[#0c0a0b] text-[#f7f3ec] pb-24">
      ...
      <div className="mx-auto max-w-6xl px-4 py-8">
        ...
        <div className="mt-8 rounded-[2rem] border border-white/5 bg-[#161314] p-2 shadow-[0_20px_40px_rgba(0,0,0,0.4)]">
        <div className="overflow-x-auto rounded-[1.5rem] border border-white/5 bg-[#161314] p-2">
          <table className="w-full border-collapse">
            <thead>
              <tr>
                <th className="border-b border-white/10 p-3 text-left text-[10px] font-black uppercase tracking-[0.25em] text-[#7d6f5c]" style={{ fontFamily: "var(--font-display)" }}>Característica</th>
                {conScore.map(p => (
                  <th key={p.id} className="min-w-[180px] border-b border-white/10 p-3 text-center">
                    {p.id === mejorId && (
                      <Badge variant="success" size="sm" className="mb-2">🏆 Mejor opción</Badge>
                    )}
                    <div className="mx-auto mb-2 flex h-24 w-24 items-center justify-center rounded-2xl border border-white/5 bg-[#161314] magenta-glow">
                      <Package className="h-10 w-10 text-[var(--accent)]" />
                    </div>
                    <p className="font-display text-sm uppercase tracking-tight">{p.name}</p>
                    <Link href={`/negocio/${p.businesses.slug}`} className="text-xs text-[#a99b86] transition hover:text-[var(--accent)]">
                      {p.businesses.name} →
                    </Link>
                  </th>
                ))}
            ...
            <tbody>
              <tr className="border-b border-white/5">
                ...
                {conScore.map(p => (
                  <td key={p.id} className="p-3 text-center">
                    {p.old_price && <p className="text-xs text-[#7d6f5c] line-through">${Number(p.old_price).toLocaleString("es-AR")}</p>}
              ...
              <tr className="border-b border-white/5">
                ...
                {conScore.map(p => (
                  <td key={p.id} className="p-3 text-center">
                    <span className={`font-display text-2xl ${p.id === mejorId ? "text-[var(--ok)]" : "text-[#f7f3ec]"}`}>
                      {p.sdlScore}
                    </span>
                    <p className="text-[10px] uppercase tracking-widest text-[#7d6f5c]">/100</p>
                  </td>
                ))}
              </tr>
              <tr>
                ...
                {conScore.map(p => (
                  <td key={p.id} className="p-3 text-center">
                    {p.id === mejorId ? (
                      ...
                      <Link href={`/negocio/${p.businesses.slug}`}
                        className="inline-block rounded-xl border border-white/10 px-4 py-2 text-xs font-black uppercase tracking-widest text-[#a99b86] transition hover:border-[var(--accent)] hover:text-white" style={{ fontFamily: "var(--font-display)" }}>
                        Ver negocio
                      </Link>
                    )}
                  </td>
...
export default function CompararPage() {
  return (
    <Suspense fallback={
      <main className="min-h-screen flex items-center justify-center bg-[#0c0a0b] text-[#f7f3ec]">
