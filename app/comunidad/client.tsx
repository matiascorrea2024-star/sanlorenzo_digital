      ...
      <div className="mx-auto max-w-4xl px-4 pb-10 pt-14 sm:px-6 md:pt-20">
        <div className="flex flex-col justify-between gap-8 md:flex-row md:items-end">
          <div className="max-w-xl">
            ...
            <h1 className="font-display text-5xl uppercase leading-[0.9] tracking-tight text-[#f7f3ec] sm:text-6xl">
              Pulso de la <span className="text-[var(--accent)]">Ciudad.</span>
            </h1>
            <p className="mt-5 text-lg text-[#a99b86]">
          ...
          {!loading && ciudades.length > 1 && (
            <div className="flex min-w-[260px] flex-col gap-2">
              <span className="ml-1 flex items-center gap-1.5 text-[10px] font-black uppercase tracking-[0.35em] text-[#7d6f5c]" style={{ fontFamily: "var(--font-display)" }}><MapPin className="h-3 w-3" /> Seleccionar localidad</span>
              <div className="rounded-2xl border border-white/10 bg-white/5 transition focus-within:border-[var(--accent)]">
                <select value={locationId} onChange={(e) => setLocationId(e.target.value)}
                  className="w-full cursor-pointer appearance-none bg-transparent px-5 py-4 text-sm font-bold text-[#f7f3ec] outline-none">
