    ...
    <main className="min-h-screen bg-[#120d09] text-white pb-24">
      <div className="mx-auto max-w-4xl px-4 py-8">
        ...
        <div className="mt-4 flex items-center justify-between">
          <h1 className="flex items-center gap-2 text-3xl font-black"><Newspaper className="h-7 w-7 text-indigo-300" /> Blog</h1>
          {!editing && (
            <button onClick={nuevo} className="flex items-center gap-1.5 rounded-xl bg-gradient-to-r from-indigo-500 to-purple-500 px-4 py-2.5 text-sm font-black hover:opacity-90">
        ...
        {editing ? (
          <div className="mt-6 space-y-4 rounded-2xl border border-white/10 bg-white/[0.03] p-5">
            <input value={draft.title} onChange={(e) => setDraft({ ...draft, title: e.target.value })}
              placeholder="Título" className="w-full rounded-xl border border-white/15 bg-black/20 px-4 py-3 font-bold outline-none focus:border-indigo-400" />
            <input value={draft.excerpt} onChange={(e) => setDraft({ ...draft, excerpt: e.target.value })}
              placeholder="Copete corto (aparece en la lista)" className="w-full rounded-xl border border-white/15 bg-black/20 px-4 py-2.5 text-sm outline-none focus:border-indigo-400" />
            <textarea value={draft.content} onChange={(e) => setDraft({ ...draft, content: e.target.value })}
              placeholder="Contenido del artículo" rows={10}
              className="w-full rounded-xl border border-white/15 bg-black/20 px-4 py-3 text-sm outline-none focus:border-indigo-400" />
            <div className="grid gap-3 sm:grid-cols-2">
              <input value={draft.cover_url} onChange={(e) => setDraft({ ...draft, cover_url: e.target.value })}
                placeholder="URL de imagen de portada (opcional)" className="rounded-xl border border-white/15 bg-black/20 px-4 py-2.5 text-sm outline-none focus:border-indigo-400" />
              <input value={draft.author} onChange={(e) => setDraft({ ...draft, author: e.target.value })}
                placeholder="Autor (opcional)" className="rounded-xl border border-white/15 bg-black/20 px-4 py-2.5 text-sm outline-none focus:border-indigo-400" />
            </div>
            ...
            <div className="flex gap-2">
              <button onClick={guardar} disabled={saving}
                className="rounded-xl bg-gradient-to-r from-indigo-500 to-purple-500 px-6 py-2.5 text-sm font-black hover:opacity-90 disabled:opacity-50">
                {saving ? "Guardando…" : "Guardar"}
              </button>
              <button onClick={() => setEditing(false)} className="rounded-xl border border-white/20 px-6 py-2.5 text-sm font-bold text-white/70 hover:bg-white/5">
                Cancelar
              </button>
            </div>
          </div>
          ...
          <div className="mt-6 space-y-2.5">
            {posts.map(p => (
              <div key={p.id} className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.02] p-4">
