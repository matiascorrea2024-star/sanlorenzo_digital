    ...
    <main className="bg-[var(--bg)] text-[var(--text)]">
      ...
      <section className="relative overflow-hidden border-b border-[var(--line)]">
        ...
        <div className="relative z-10 mx-auto max-w-4xl px-4 py-20 text-center md:py-28">
          ...
          <p className="mx-auto mt-6 max-w-xl text-base leading-relaxed text-[var(--muted)] md:text-lg">Miniweb propia, productos con fotos, promociones que se renuevan solas y contacto directo por WhatsApp. Sin saber programar.</p>
      ...
      <section className="mx-auto max-w-6xl px-4 py-16 md:py-24">
        ...
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {BENEFICIOS.map((b) => (
            <div key={b.t} className="rounded-[2rem] border border-[var(--line)] bg-[var(--surface)] p-7 transition-colors duration-300 hover:border-[var(--accent)]/40">
              <div className={`flex h-12 w-12 items-center justify-center rounded-2xl border border-[var(--line-strong)] bg-gradient-to-br ${b.grad} text-2xl shadow-lg`}>{b.i}</div>
              <h3 className="mt-5 font-display text-lg tracking-tight">{b.t}</h3>
              <p className="mt-2 text-sm leading-relaxed text-[var(--muted)]">{b.d}</p>
            </div>
          ))}
        </div>
      </section>
          ...
          <div className="grid gap-5 sm:grid-cols-2">
            {ejemplos.map((b: any) => (
              <a key={b.slug} href={`/negocio/${b.slug}`}
                className="group overflow-hidden rounded-[2rem] border border-[var(--line)] bg-[var(--surface)] transition-all duration-300 hover:-translate-y-1 hover:border-[var(--accent)]/50">
                <div className="relative h-32 w-full overflow-hidden border-b border-[var(--line)]">
                ...
                <div className="p-6">
                  ...
                  <p className="mt-1 text-sm capitalize text-[var(--muted)]">{b.category} · ✓ verificado</p>
                </div>
              </a>
            ))}
          </div>
