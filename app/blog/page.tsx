    ...
    <main className="min-h-screen bg-[var(--bg)] text-[var(--text)] pb-24">
      ...
      <div className="mx-auto max-w-4xl px-4 py-10">
        {!posts || posts.length === 0 ? (
          <div className="rounded-3xl border border-dashed border-[var(--line-strong)] bg-[var(--surface)] p-12 text-center">
            <Newspaper className="mx-auto h-10 w-10 text-[var(--muted2)]" />
            <p className="mt-3 text-sm text-[var(--muted)]">Todavía no publicamos artículos. Volvé pronto.</p>
          </div>
        ) : (
          <div className="space-y-4">
            {posts.map((p: any) => (
              <Link key={p.id} href={`/blog/${p.slug}`}
                className="group block rounded-[2rem] border border-[var(--line)] bg-[var(--surface)] p-5 transition-all duration-700 ease-[cubic-bezier(0.165,0.84,0.44,1)] hover:-translate-y-2 hover:border-[var(--accent)] hover:shadow-[0_20px_40px_rgba(0,0,0,0.6),0_0_20px_rgba(209,47,104,0.1)]">
                {p.cover_url && (
                  <div className="relative mb-4 h-48 w-full overflow-hidden rounded-[1.25rem] border border-[var(--line)]">
                ...
                <p className="mt-3 text-xs uppercase tracking-widest text-[var(--muted2)]">
